"""Hierarchical action term for the G1 assist-exoskeleton task."""
...
class FrozenGaitAssistTorqueAction(ActionTerm):
    ...
    def __init__(self, cfg: FrozenGaitAssistTorqueActionCfg, env):
        ...
        if resolved_policy_names != cfg.policy_joint_names:
            raise RuntimeError(
                f"Frozen gait joint order mismatch: expected {cfg.policy_joint_names}, got {resolved_policy_names}"
            )
        if resolved_assist_names != cfg.assist_joint_names:
            raise RuntimeError(
                f"Assist joint order mismatch: expected {cfg.assist_joint_names}, got {resolved_assist_names}"
            )
        ...
        self._gait_actions = torch.zeros(self.num_envs, len(POLICY_JOINT_NAMES), device=self.device)
        self._previous_gait_actions = torch.zeros_like(self._gait_actions)
        self._gait_position_targets = self._asset.data.default_joint_pos[:, self._policy_joint_ids].clone()
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        self._processed_actions[:] = torch.clamp(actions, -1.0, 1.0) * self.cfg.assist_torque_limit
        ...
        if gait_actions.shape[-1] != len(self._policy_joint_ids):
            raise RuntimeError(
                f"Frozen gait policy expects 29 outputs, got action shape {tuple(gait_actions.shape)}"
            )
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        # Select environments and joints in two steps. Using
        # default_joint_pos[env_ids, joint_ids] invokes paired advanced indexing
        # and fails when num_reset_envs != num_policy_joints (for example
        # 6000 environments versus 29 joints).
...
class FrozenGaitAssistTorqueActionCfg(ActionTermCfg):
    """Configuration for :class:`FrozenGaitAssistTorqueAction`."""
