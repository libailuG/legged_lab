...
class CompletedPulsePeakReward(ManagerTermBase):
    ...
    def __init__(self, cfg, env):
        ...
        self.response = ResponseMetrics(env.num_envs, env.device, env.step_dt)
    ...
    def __call__(self, env, hip_cfg, sensor_cfg, action_name="assist_torque", torque_limit=10.):
        ...
        desired_peak = torch.minimum(desired_peak, pulse.feasible_peak)
        ...
        log.update(self.response.update(velocity, pulse))
        log["Response/displacement_blocked_fraction"] = pulse.blocked.float().mean()
        log["Response/cadence_valid_fraction"] = (pulse.state[:,17]>0).float().mean()
        log["Response/peak_cap_start_fraction"] = pulse.peak_capped.sum()/starts.sum().clamp_min(1)
        log["Response/estimated_step_interval_s"] = (pulse.state[:,14]*pulse.state[:,17]).sum()/pulse.state[:,17].sum().clamp_min(1)
...
def standing_false_start(env, hip_cfg, action_name="assist_torque"):
    """Training-only false-onset event cost, including zero-peak triggers."""
    command = env.command_manager.get_command("base_velocity")
    standing = torch.linalg.vector_norm(command, dim=-1) < .01
    robot = env.scene[hip_cfg.name]
    quiet = robot.data.joint_vel[:,hip_cfg.joint_ids].abs().amax(-1) < .12
    slow_base = torch.linalg.vector_norm(robot.data.root_lin_vel_b[:,:2],dim=-1) < .15
    start = env.action_manager.get_term(action_name)._pulse.started
    return (standing & quiet & slow_base & start).float()/env.step_dt
