"""Paired G1 exoskeleton assistance with training-only privileged contact information."""
...
# TorchScript actor verified against standing_reinforcement/model_51800.pt.
# Keep a checkpoint-specific export so later policy.pt exports cannot replace it.
FROZEN_GAIT_POLICY_PATH = os.path.abspath(
    os.path.join(
        ...
        "rsl_rl",
        "g1_assist_v1_amp",
        "2026-09-07_09-57-24_standing_reinforcement",
        "exported",
        "policy_51800.pt",
    )
)
...
FOOT_CONTACT_CFG = SceneEntityCfg(
    "contact_forces",
    body_names=["left_ankle_roll_link", "right_ankle_roll_link"],
    preserve_order=True,
)
ASSIST_TORQUE_LIMIT = 10.0
ASSIST_TORQUE_RATE_LIMIT = 80.0
# These are tunable training-reference thresholds, not validated human comfort limits.
SUPPORT_REWARD_PARAMS = {
    "hip_cfg": HIP_PITCH_JOINT_CFG,
    "sensor_cfg": FOOT_CONTACT_CFG,
    "action_name": "assist_torque",
    "torque_limit": ASSIST_TORQUE_LIMIT,
    "support_force_off": 10.0,
    "support_force_on": 20.0,
    "support_force_full": 200.0,
    "support_rise_time": 0.05,
...
class AssistActionsCfg:
    """Peak and duration are latched at the start of each paired pulse."""

    assist_torque = mdp.FrozenGaitAssistTorqueActionCfg(
        asset_name="robot",
        frozen_policy_path=FROZEN_GAIT_POLICY_PATH,
        torque_limit=ASSIST_TORQUE_LIMIT,
        torque_rate_limit=ASSIST_TORQUE_RATE_LIMIT,
        motion_speed_deadzone=0.15,
        motion_speed_full=0.8,
        motion_filter_time_constant=0.03,
    )


@configclass
class AssistPolicyObservationGroup(ObsGroup):
    """Deployable history of local encoders, torque, and pulse-generator state."""

    assist_joint_pos = ObsTerm(
        func=mdp.biased_assist_position,
    ...
    pulse_state = ObsTerm(func=mdp.pulse_controller_state)
...
class AssistCriticObservationGroup(AssistPolicyObservationGroup):
    ...
    # Critic/rewards retain physical angles; the unknown encoder bias is not exposed to Actor.
    assist_joint_pos = ObsTerm(func=mdp.joint_pos, params={"asset_cfg": ASSIST_JOINT_CFG})

    # Training-only foot information. Never add this term to the actor or
    # use it in the deployable action processing path.
    foot_support = ObsTerm(
        func=mdp.privileged_foot_support,
        params={
            "sensor_cfg": FOOT_CONTACT_CFG,
...
class AssistRewardsCfg:
    """Support-aware paired lift, no unsupported pressing, and rear-mechanism stability."""
    ...
    assist_requested_rate_excess = None
    ...
    rear_mechanism_velocity = RewTerm(
        func=mdp.rear_mechanism_velocity_l2,
        weight=-0.5,
        params={
            "asset_cfg": REAR_MECHANISM_JOINT_CFG,
            "slider_velocity_scale": 0.2,
            "cylinder_velocity_scale": 1.0,
        },
    )
    assist_torque_zero_at_bilateral_rest = RewTerm(
        func=mdp.SupportAwareAssistReward, weight=-2.0,
        params={**SUPPORT_REWARD_PARAMS, "mode": "standing_rest"},
    )
    assist_completed_pulse_peak = RewTerm(
        func=mdp.CompletedPulsePeakReward, weight=-6.0,
        params={"hip_cfg": HIP_PITCH_JOINT_CFG, "sensor_cfg": FOOT_CONTACT_CFG},
    )
    assist_standing_false_start = RewTerm(
        func=mdp.standing_false_start, weight=-4.0,
        params={"hip_cfg": HIP_PITCH_JOINT_CFG},
    )
    assist_action_bounds = RewTerm(
        func=mdp.assist_action_out_of_bounds, weight=-0.2,
    )
    assist_terminal_support = RewTerm(
        func=mdp.SupportAwareAssistReward, weight=-6.0,
        params={**SUPPORT_REWARD_PARAMS, "mode": "posture_support"},
    )
    assist_supported_lift_tracking = None
    assist_supported_positive_work = RewTerm(
        func=mdp.SupportAwareAssistReward, weight=4.0,
        params={**SUPPORT_REWARD_PARAMS, "mode": "supported_work"},
    )
    assist_lowering_resistance = RewTerm(
        func=mdp.SupportAwareAssistReward, weight=-4.0,
        params={**SUPPORT_REWARD_PARAMS, "mode": "lowering_resistance"},
    )
    assist_unsupported_press = RewTerm(
        func=mdp.SupportAwareAssistReward,
        weight=-8.0,
        params={**SUPPORT_REWARD_PARAMS, "mode": "unsupported_press"},
    )
    hip_motor_torque_burden = RewTerm(
        func=mdp.hip_motor_torque_burden_l2, weight=-0.2,
        params={"hip_cfg": HIP_PITCH_JOINT_CFG},
    )
...
@configclass
class G1AssistExoskeletonV2V8EnvCfg(G1AssistAmpEnvCfg):
    """Train v2-v8 with -10 Nm lift and +10 Nm press authority."""
    ...
    def __post_init__(self):
        ...
        # Keep the 1.92 s rollout within a 16 GiB GPU memory budget.
        ...
        for actuator in self.scene.robot.actuators.values():
            if hasattr(actuator, "integration_dt"):
                actuator.integration_dt = self.sim.dt
        self.decimation = 10
        self.episode_length_s = 20.0
        # Sample standing commands with 10% probability.
        self.commands.base_velocity.rel_standing_envs = 0.10
        # Match the requested body forward-command range in training and Play.
        self.commands.base_velocity.ranges.lin_vel_x = (0.4, 3.0)
...
@configclass
class G1AssistExoskeletonV2V8EnvCfg_PLAY(G1AssistExoskeletonV2V8EnvCfg):
    """Play configuration for the asymmetric v2-v8 assist policy."""

    def __post_init__(self):
        super().__post_init__()
        self.actions.assist_torque.gait_observation_noise = False
        self.actions.assist_torque.angle_bias_range = 0.0
