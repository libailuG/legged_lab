"""Frozen gait plus low-speed rise-hold-release paired assistance."""
...
class FrozenGaitAssistTorqueAction(ActionTerm):
    ...
    def __init__(self, cfg: FrozenGaitAssistTorqueActionCfg, env):
        ...
        self._pulse = SinglePulse(self.num_envs, self.device, env.step_dt,
                                  cfg.pulse_rise_time, cfg.pulse_release_duration,
                                  cfg.pulse_duration_min, cfg.pulse_duration_max,
                                  cfg.torque_limit, cfg.torque_rate_limit, cfg.pulse_confirm_time)
        self._angle_bias = torch.zeros(self.num_envs, 2, device=self.device)
        ...
        self._motion_gate = torch.zeros(self.num_envs, 1, device=self.device)
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        scalar = self._pulse.step(actions, self._filtered_assist_joint_velocity,
                                  self._asset.data.joint_pos[:, self._assist_joint_ids] + self._angle_bias)
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        selected = self._angle_bias[env_ids]
        self._angle_bias[env_ids] = (2 * torch.rand_like(selected) - 1) * self.cfg.angle_bias_range
...
class FrozenGaitAssistTorqueActionCfg(ActionTermCfg):
    """Two normalized actions -> latched peak/duration for a paired rise-hold-release pulse."""
    ...
    pulse_rise_time: float = 0.20
    pulse_release_duration: float = 0.25
    pulse_duration_min: float = 0.6
    pulse_duration_max: float = 1.4
    pulse_confirm_time: float = 0.03
    angle_bias_range: float = math.radians(10.0)
    ...
    motion_filter_time_constant: float = 0.03
...
def biased_assist_position(env, asset_cfg, action_name="assist_torque"):
    term = env.action_manager.get_term(action_name)
    return env.scene[asset_cfg.name].data.joint_pos[:, asset_cfg.joint_ids] + term._angle_bias
