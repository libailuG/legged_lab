"""Frozen gait plus one deployable signed action controlling an equal/opposite torque pair."""
...
class FrozenGaitAssistTorqueAction(ActionTerm):
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        target_scalar = actions.clamp(-1.0, 1.0) * self.cfg.torque_limit * self._motion_gate
        self._previous_processed_actions[:] = self._processed_actions
        self._requested_torque_delta[:] = paired_torques(target_scalar) - self._processed_actions
        scalar = slew_paired_scalar(
            target_scalar, self._processed_actions[:, :1], self.cfg.torque_limit,
            self.cfg.torque_rate_limit, self._env.step_dt,
        )
...
class FrozenGaitAssistTorqueActionCfg(ActionTermCfg):
    """One normalized signed action -> [T, -T], with shared motion gate and slew limit."""
