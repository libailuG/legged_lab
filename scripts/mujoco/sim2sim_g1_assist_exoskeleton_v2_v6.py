"""Isaac v6 -> MuJoCo: frozen gait + latched peak/duration paired hip assistance.
...
def load_training_config(path):
    ...
    if 'g1_assist_exoskeleton_v2_v6' not in cfg['actions']['assist_torque']['class_type']:
        raise ValueError('Expected v6 training configuration')
    return cfg
...
class PairedAssistController:
    ...
    def __init__(self, cfg, dt):
        ...
        path = PROJECT_ROOT / 'source/legged_lab/legged_lab/tasks/locomotion/amp/config/g1_assist_exoskeleton_v2_v6/mdp/pulse.py'
        spec = importlib.util.spec_from_file_location('v6_pulse', path)
...
class LivePlot:
    ...
    def __init__(self, window, interval, dt, command):
        ...
        self.fig.canvas.manager.set_window_title('G1 v6 - hip / assist / foot contact')
        ...
        self.rear_fig.canvas.manager.set_window_title('G1 v6 - rear mechanism / forward speed')
        ...
        self.control_fig.canvas.manager.set_window_title('G1 v6 - speed control')
            ...
            for col, side in enumerate(SIDES):
                ...
                for row_index, ax in enumerate(self.axes[:, col]):
                    ...
                    if row_index == 2:
                        ax.set_ylim(-1.1, 1.1)
                        ax.legend(loc='upper right', fontsize=7)
            self.fig.suptitle(f"G1 v6 | t={t[-1]:.2f} s | episode={rows[-1]['episode']} | vx={rows[-1]['actual_vx_m_s']:.2f} m/s")
...
def main():
    ...
    parser.add_argument('--env-config', type=Path, required=False, default=PROJECT_ROOT / 'logs/rsl_rl/g1_assist_exoskeleton_v2_v6_ppo/2026-09-09_14-00-18_stronger_support/params/env.yaml', help="New PID training run's params/env.yaml")
    ...
    with torch.inference_mode():
        ...
        if assist_policy is not None and tuple(assist_policy(torch.zeros(1, 450)).shape) != (1, 2):
            raise ValueError('v6 assist policy must map 450 -> 2 (peak, duration); previous task exports are incompatible')
