#!/usr/bin/env python3
"""Run the assist-v1 gait policy with PID and the v2-v2 exoskeleton policy.

Gait inference: 50 Hz; assist inference: 100 Hz; physics and PID: 1 kHz.
The exoskeleton XML is retained to provide the assist and rear mechanism joints.
"""
...
from sim2sim_g1_assist_v1 import (
    DEFAULT_POLICY as V1_GAIT_POLICY,
    DECIMATION as GAIT_DECIMATION,
    pid_torque,
    configure_actuators,
)
...
DEFAULT_GAIT_POLICY = V1_GAIT_POLICY
DEFAULT_ASSIST_POLICY = (
    PROJECT_ROOT
    / "logs/rsl_rl/g1_assist_exoskeleton_v2_v2_ppo/2026-09-04_17-54-31_lift10_rate80_press4_rate40_motion_gate_resume/exported/policy.pt"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Reset above this angle error in degrees; disabled by default (0).",
    )
...
def main() -> None:
    ...
    configure_actuators(model, indices["policy_qvel"], indices["policy_actuator"])
    ...
    output_dir = (
        ...
        if args.output_dir is not None
            ...
            PROJECT_ROOT / "logs/sim2sim_pid_assist_exoskeleton_v2_v2_no_assist"
            if assist_policy is None
            else (
                assist_numpy_policy_path.parent.parent / "output"
                if assist_numpy_policy_path is not None
                else assist_policy_path.parent.parent / "sim2sim_pid_assist_exoskeleton_v2_v2"
            )
        )
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    csv_path = output_dir / "g1_pid_assist_exoskeleton_v2_v2_sim2sim.csv"
    png_path = output_dir / "g1_pid_assist_exoskeleton_v2_v2_sim2sim.png"
    signal_names = ["time"] + [
        f"{side}_{signal}"
        for side in SIDES
        for signal in (
    ...
    print(
        f"Torque target: -{ASSIST_LIFT_TORQUE_LIMIT:g} Nm lift / "
    ...
    print("Gait control:  assist-v1 policy 50 Hz / PID 1000 Hz; assist policy 100 Hz")
    print("Assist physical limit: XML retains +/-8 Nm, matching this assist checkpoint's training")
    ...
    try:
        while viewer is None or viewer.is_running():
            ...
            if step % DECIMATION == 0:
                ...
                if control_step % (GAIT_DECIMATION // DECIMATION) == 0:
            ...
            gait_torque = pid_torque(gait_target, joint_pos, joint_vel, integral_effort)
