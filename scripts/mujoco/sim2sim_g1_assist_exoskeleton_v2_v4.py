"""Isaac v4 -> MuJoCo: frozen gait + one-action paired hip assistance.
...
def load_training_config(path):
    ...
    if 'g1_assist_exoskeleton_v2_v4' not in cfg['actions']['assist_torque']['class_type']:
        raise ValueError('Expected v4 training configuration')
    return cfg
...
class AssistHistory:
    """Term-major 400-vector: 25 oldest-to-newest [L,R] samples for q, qd, torque."""

    def __init__(self, length=25):
        self.data = np.zeros((3, length, 2), dtype=np.float32)
        self.pulse_data = np.zeros((length, 10), dtype=np.float32)
...
class PairedAssistController:
    ...
    def __init__(self, cfg, dt):
        ...
        path = PROJECT_ROOT / 'source/legged_lab/legged_lab/tasks/locomotion/amp/config/g1_assist_exoskeleton_v2_v4/mdp/pulse.py'
        spec = importlib.util.spec_from_file_location('v4_pulse', path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        self.pulse = module.SinglePulse(1, 'cpu', dt, cfg['pulse_duration'], cfg['pulse_release_duration'],
    ...
    def step(self, action, velocity):
        ...
        scalar=self.pulse.step(t.tensor([[action]]),self.filtered_velocity,gate)
...
class LivePlot:
    ...
    def __init__(self, window, interval, dt, command):
        ...
        self.fig.canvas.manager.set_window_title('G1 v4 - hip / assist / foot contact')
        ...
        self.rear_fig.canvas.manager.set_window_title('G1 v4 - rear mechanism / forward speed')
        ...
        self.control_fig.canvas.manager.set_window_title('G1 v4 - speed control')
            ...
            for col, side in enumerate(SIDES):
                ...
                for row_index, ax in enumerate(self.axes[:, col]):
                    ...
                    if row_index == 2:
                        ax.set_ylim(-1.1, 1.1)
                        ax.legend(loc='upper right', fontsize=7)
            self.fig.suptitle(f"G1 v4 | t={t[-1]:.2f} s | episode={rows[-1]['episode']} | vx={rows[-1]['actual_vx_m_s']:.2f} m/s")
...
def main():
    ...
    parser.add_argument('--env-config', type=Path, required=False, default=PROJECT_ROOT / 'logs/rsl_rl/g1_assist_exoskeleton_v2_v4_ppo/2026-09-08_15-42-59_single_pulse_04s/params/env.yaml', help="New PID training run's params/env.yaml")
    ...
    with torch.inference_mode():
        ...
        if assist_policy is not None and tuple(assist_policy(torch.zeros(1, 400)).shape) != (1, 1):
            raise ValueError('v4 assist policy must map 400 -> 1; v2 two-action exports are incompatible')
    ...
    try:
        ...
        while viewer.is_running() and live_plot.is_running():
            ...
            with torch.inference_mode():
                raw_action = -1.0 if assist_policy is None else float(assist_policy(torch.from_numpy(history.observation()).unsqueeze(0)).item())
            ...
            row = dict(time_s=total_steps * sim_dt, episode=episode, episode_time_s=float(data.time), command_vx_m_s=float(command[0]), command_vy_m_s=float(command[1]), command_yaw_rad_s=float(command[2]), actual_vx_m_s=actual_vx, base_height_m=float(data.qpos[2]), raw_assist_action=raw_action, shared_gate=controller.gate, assist_pair_sum_nm=float(actual_torque.sum()), rear_slider_m=float(data.qpos[rear['qpos']][0]), rear_slider_velocity_m_s=float(data.qvel[rear['qvel']][0]), rear_cylinder_deg=float(np.rad2deg(data.qpos[rear['qpos']][1])), rear_cylinder_velocity_deg_s=float(np.rad2deg(data.qvel[rear['qvel']][1])), termination=reason)
