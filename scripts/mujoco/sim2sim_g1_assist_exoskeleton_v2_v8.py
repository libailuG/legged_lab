"""Isaac v8 -> MuJoCo: frozen gait + latched peak/duration paired hip assistance.
...
def load_training_config(path):
    ...
    if 'g1_assist_exoskeleton_v2_v8' not in cfg['actions']['assist_torque']['class_type']:
        raise ValueError('Expected v8 training configuration')
    return cfg
...
class AssistHistory:
    """Term-major 650-vector: 25 oldest-to-newest [L,R] samples for q, qd, torque."""

    def __init__(self, length=25):
        self.data = np.zeros((3, length, 2), dtype=np.float32)
        self.pulse_data = np.zeros((length, 20), dtype=np.float32)
...
class PairedAssistController:
    ...
    def __init__(self, cfg, dt):
        ...
        path = PROJECT_ROOT / 'source/legged_lab/legged_lab/tasks/locomotion/amp/config/g1_assist_exoskeleton_v2_v8/mdp/pulse.py'
        spec = importlib.util.spec_from_file_location('v8_pulse', path)
    ...
    def step(self, action, velocity, position):
        if np.asarray(action).shape != (2,) or not np.isfinite(action).all() or not np.isfinite(velocity).all() or np.asarray(position).shape != (2,) or not np.isfinite(position).all():
        ...
        scalar=self.pulse.step(t.as_tensor(action, dtype=t.float32).reshape(1,2),self.filtered_velocity,t.as_tensor(position,dtype=t.float32).reshape(1,2))
...
class LivePlot:
    ...
    def __init__(self, window, interval, dt, command):
        ...
        self.fig.canvas.manager.set_window_title('G1 v8 - hip / assist / foot contact')
        ...
        self.rear_fig.canvas.manager.set_window_title('G1 v8 - rear mechanism / forward speed')
        ...
        self.control_fig.canvas.manager.set_window_title('G1 v8 - speed control')
            ...
            for col, side in enumerate(SIDES):
                ...
                for row_index, ax in enumerate(self.axes[:, col]):
                    ...
                    if row_index == 2:
                        ax.set_ylim(-1.1, 1.1)
                        ax.legend(loc='upper right', fontsize=7)
            self.fig.suptitle(f"G1 v8 | t={t[-1]:.2f} s | episode={rows[-1]['episode']} | vx={rows[-1]['actual_vx_m_s']:.2f} m/s")
...
def main():
    ...
    parser.add_argument('--env-config', type=Path, required=False, default=PROJECT_ROOT / 'logs/rsl_rl/g1_assist_exoskeleton_v2_v8_ppo/2026-09-10_13-38-04_motion_guard_cadence/params/env.yaml', help="New PID training run's params/env.yaml")
    ...
    with torch.inference_mode():
        ...
        if assist_policy is not None and tuple(assist_policy(torch.zeros(1, 650)).shape) != (1, 2):
            raise ValueError('v8 assist policy must map 650 -> 2 (peak, duration); previous task exports are incompatible')
    ...
    try:
        ...
        while viewer.is_running() and live_plot.is_running():
            ...
            torques = controller.step(raw_action, data.qvel[assist['qvel']], data.qpos[assist['qpos']])
            steps = decimation
