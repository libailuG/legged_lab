#!/usr/bin/env python3
"""Isaac v3 -> MuJoCo: frozen gait + one-action paired hip assistance.
...
def load_training_config(path):
    ...
    if "g1_assist_exoskeleton_v2_v3" not in cfg["actions"]["assist_torque"]["class_type"]:
        raise ValueError("Expected v3 training configuration")
    return cfg
...
class AssistHistory:
    """Term-major 150-vector: 25 oldest-to-newest [L,R] samples for q, qd, torque."""

    def __init__(self, length=25):
        self.data = np.zeros((3, length, 2), dtype=np.float32)

    def reset(self, position, velocity):
        # Isaac's first buffer push fills every history slot with that sample.
        self.data[0, :, :] = position
        self.data[1, :, :] = velocity
        self.data[2, :, :] = 0.0

    def append(self, position, velocity, torque):
...
class PairedAssistController:
    """NumPy implementation of v3's deployable postprocessing, independent of Isaac."""
    ...
    def reset(self):
        ...
        self.scalar = self.gate = 0.0

    def step(self, action, velocity):
        if not math.isfinite(action) or not np.isfinite(velocity).all():
            raise ValueError("Non-finite assist input")
        c = self.cfg
        alpha = 1.0 - math.exp(-self.dt / c["motion_filter_time_constant"])
        self.filtered_velocity += alpha * (velocity - self.filtered_velocity)
        phase = np.clip((np.max(np.abs(self.filtered_velocity)) - c["motion_speed_deadzone"])
                        / (c["motion_speed_full"] - c["motion_speed_deadzone"]), 0.0, 1.0)
        self.gate = float(phase * phase * (3.0 - 2.0 * phase))
        target = float(np.clip(action, -1.0, 1.0)) * c["torque_limit"] * self.gate
        if target * self.scalar < 0.0:
            target = 0.0
        step_limit = c["torque_rate_limit"] * self.dt
        self.scalar = float(np.clip(self.scalar + np.clip(target - self.scalar, -step_limit, step_limit),
                                    -c["torque_limit"], c["torque_limit"]))
        return np.array((self.scalar, -self.scalar))
...
class JointPIDController:
    ...
    def step(self, target, position, velocity):
        ...
        candidate = np.clip(self.integral + g["ki"] * error * self.dt,
                            -g["integral_limits"], g["integral_limits"])
        effort = pd + candidate
        clipped = np.clip(effort, -g["pid_limits"], g["pid_limits"])
        reject = (effort != clipped) & (error * effort > 0.)
...
def configure_model(mj, model, cfg):
    ...
    groups = {}
    for group, key in (("gait", "policy_joint_names"), ("assist", "assist_joint_names"),
                       ("rear", "extra_position_joint_names")):
        names = action[key]
        ...
        for i, name in enumerate(names):
            matches = [a for a in robot_cfg["actuators"].values()
        ...
        model.dof_damping[qvel] = 0.0  # PD below accounts for damping; do not add XML damping twice.
        ...
        groups[group] = dict(names=names, joints=joints, motors=motors, qpos=qpos, qvel=qvel,
                             kp=kp, kd=kd, ki=ki, integral_limits=integral_limits,
                             pid_limits=pid_limits, limits=limits,
                             default=resolve_parameter(robot_cfg["init_state"]["joint_pos"], names))
    ...
    # Verified against v3 USD physics: slider +/-0.05 m, cylinder +/-0.2 rad.
    # Legacy XML contains +/-0.01 m and +/-pi instead.
    model.jnt_range[groups["rear"]["joints"]] = [[-.05, .05], [-.2, .2]]
...
def foot_normal_forces(mj, model, data, feet, floor):
    ...
    for idx in range(data.ncon):
        ...
        # Contact frame row 0 is the normal from geom1 to geom2.
...
class LivePlot:
    ...
    def __init__(self, window, interval, dt, command):
        ...
        self.fig.canvas.manager.set_window_title("G1 v3 - hip / assist / foot contact")
        signals = (("hip_deg", "Angle (deg)"), ("hip_velocity_deg_s", "Velocity (deg/s)"),
                   ("hip_torque_nm", "Torque / window peak"), ("foot_fz_n", "Foot Fz (N)"))
        for col, side in enumerate(SIDES):
            for row, (signal, label) in enumerate(signals):
                ...
                if row < 2:
                    ...
                    self.lines[f"{side}_{extra}"], = ax.plot([], [], lw=1, alpha=.8, label="Exoskeleton")
                if row == 2:
        ...
        self.fig.tight_layout(rect=(0, 0, 1, .97))
        self.rear_fig, self.rear_axes = plt.subplots(3, 1, figsize=(9, 6), sharex=True)
        self.rear_fig.canvas.manager.set_window_title("G1 v3 - rear mechanism / forward speed")
        for ax, (key, label) in zip(self.rear_axes, (("rear_slider_m", "Slider (m)"),
                                                   ("rear_cylinder_deg", "Cylinder (deg)"),
                                                   ("actual_vx_m_s", "vx (m/s)"))):
        ...
        self.control_fig.canvas.manager.set_window_title("G1 v3 - speed control")
        self.control_fig.text(.5, .78, "Forward speed command", ha="center", fontsize=13)
        slider_ax = self.control_fig.add_axes((.20, .38, .65, .13))
        self.speed_slider = Slider(
            slider_ax, "vx (m/s)", -.5, 3., valinit=float(command[0]),
            valstep=.05, valfmt="%.2f",
        )
    ...
    def update(self, row=None, force=False):
        ...
        if not main_open and not rear_open:
            return
        # Keyboard changes share the command array; synchronize without firing callbacks.
        if control_open and not math.isclose(self.speed_slider.val, float(self.command[0]), abs_tol=1e-6):
        ...
        xmin, xmax = max(0., t[-1] - self.window), max(self.window, t[-1])
        if main_open:
            for col, side in enumerate(SIDES):
                ...
                for row_index, ax in enumerate(self.axes[:, col]):
                    ...
                    self.shading[key] = ax.fill_between(
                        t, 0, 1, where=contact, step="post", transform=ax.get_xaxis_transform(),
                        color="green", alpha=.10,
                    )
                    ax.relim()
                    ...
                    if row_index == 2:
                        ax.set_ylim(-1.1, 1.1)
                        ax.legend(loc="upper right", fontsize=7)
            self.fig.suptitle(f"G1 v3 | t={t[-1]:.2f} s | episode={rows[-1]['episode']} | vx={rows[-1]['actual_vx_m_s']:.2f} m/s")
    ...
    def is_running(self):
        return all(self.plt.fignum_exists(fig.number)
                   for fig in (self.fig, self.rear_fig, self.control_fig))
...
def main(numpy_deployment=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--assist-policy", type=Path, default=None,
                        help="Exported TorchScript actor including observation normalization; not model_N.pt")
    parser.add_argument("--env-config", type=Path, required=numpy_deployment is None,
                        default=None if numpy_deployment is None else numpy_deployment / "env.yaml", help="New PID training run's params/env.yaml")
    parser.add_argument("--gait-policy", type=Path, default=None)
    parser.add_argument("--model", type=Path, default=DEFAULT_MODEL)
    parser.add_argument("--disable-assist", action="store_true")
    parser.add_argument("--vx", type=float, default=.7)
    parser.add_argument("--vy", type=float, default=0.)
    parser.add_argument("--yaw", type=float, default=0.)
    parser.add_argument("--no-realtime", action="store_true")
    parser.add_argument("--plot-window", type=float, default=5., help="Seconds visible in the live charts")
    parser.add_argument("--plot-interval", type=float, default=.1, help="Minimum wall-clock seconds between chart refreshes")
    ...
    if numpy_deployment is not None:
        import sys
        if str(PROJECT_ROOT) not in sys.path:
            sys.path.insert(0, str(PROJECT_ROOT))
        from sim2real.g1_assist_exoskeleton_v3.numpy_gait_policy import NumpyGaitPolicy
        from sim2real.g1_assist_exoskeleton_v3.controller import AssistController
        if args.gait_policy is not None or args.assist_policy != args.env_config.resolve().parent.parent / "exported/policy.pt":
            parser.error("NumPy mode uses bundled weights; change --weights-dir in the NumPy entry point")
        gait_policy = NumpyGaitPolicy(numpy_deployment)
        deployment_controller = AssistController(numpy_deployment)
        for key, value in deployment_controller.cfg.items():
            expected = dt if key == "dt" else action_cfg[key]
            if not math.isclose(value, expected, rel_tol=0., abs_tol=1e-12):
                raise ValueError(f"Deployment and simulation control configuration differ: {key}")
        assist_policy = None if args.disable_assist else deployment_controller.policy
        gait_path = numpy_deployment / "gait_policy.npz"
        args.assist_policy = numpy_deployment / "assist_policy.npz"
    else:
        ...
        with torch.inference_mode():
            ...
            if assist_policy is not None and tuple(assist_policy(torch.zeros(1, 150)).shape) != (1, 1):
                raise ValueError("v3 assist policy must map 150 -> 1; v2 two-action exports are incompatible")
    ...
    controller = deployment_controller if numpy_deployment is not None else PairedAssistController(action_cfg, dt)
    ...
    def reset():
        ...
        if numpy_deployment is not None:
            controller.reset(data.qpos[assist["qpos"]], data.qvel[assist["qvel"]])
        else:
            controller.reset()
        body_pid.reset()
    ...
    def key_callback(keycode):
        ...
        changes = {"W": (0,.1), "S": (0,-.1), "A": (1,.1), "D": (1,-.1), "Q": (2,.1), "E": (2,-.1)}
        if key in changes:
            i, delta = changes[key]
            command[i] += delta
            command[:] = np.clip(command, [-.5,-.5,-1.], [3.,.5,1.])
        elif key == " ":
            command.fill(0.)
        elif key == "R":
    ...
    try:
        ...
        viewer.cam.distance, viewer.cam.azimuth, viewer.cam.elevation = 3.5, 135., -20.
        ...
        while viewer.is_running() and live_plot.is_running():
            ...
            if gait_tick == 0:
                ...
                gait_obs = np.concatenate((body_velocity[:3], rotation.T @ [0.,0.,-1.], command,
                                           data.qpos[gait["qpos"]] - gait["default"],
                                           data.qvel[gait["qvel"]], last_gait_action)).astype(np.float32)
                # Playback uses clean body observations, even with a training YAML.
                if numpy_deployment is not None:
                    gait_action = gait_policy(gait_obs)
                else:
                    with torch.inference_mode():
            ...
            if numpy_deployment is not None:
                torques = controller.step(data.qpos[assist["qpos"]], data.qvel[assist["qvel"]],
                                          enabled=not args.disable_assist)
                raw_action = controller.raw_action
            else:
                with torch.inference_mode():
                    raw_action = 0.0 if assist_policy is None else float(assist_policy(
                        torch.from_numpy(history.observation()).unsqueeze(0)).item())
            ...
            total_steps += steps
            # Synchronize pose, sensor and contact telemetry at the end of the control interval.
            mj.mj_forward(model, data)
            if not np.isfinite(data.qpos).all() or not np.isfinite(data.qvel).all():
                raise RuntimeError("Non-finite MuJoCo state")
            history.append(data.qpos[assist["qpos"]], data.qvel[assist["qvel"]], torques)
            fz = foot_normal_forces(mj, model, data, feet, floor)
            actual_torque = data.qfrc_actuator[assist["qvel"]].copy()
            if abs(actual_torque.sum()) > 1e-5 or np.max(np.abs(actual_torque)) > action_cfg["torque_limit"] + 1e-5:
            ...
            row = dict(time_s=total_steps*sim_dt, episode=episode, episode_time_s=float(data.time),
                       command_vx_m_s=float(command[0]), command_vy_m_s=float(command[1]), command_yaw_rad_s=float(command[2]),
                       actual_vx_m_s=actual_vx, base_height_m=float(data.qpos[2]), raw_assist_action=raw_action,
                       shared_gate=controller.gate, assist_pair_sum_nm=float(actual_torque.sum()),
                       rear_slider_m=float(data.qpos[rear["qpos"]][0]),
                       rear_slider_velocity_m_s=float(data.qvel[rear["qvel"]][0]),
                       rear_cylinder_deg=float(np.rad2deg(data.qpos[rear["qpos"]][1])),
                       rear_cylinder_velocity_deg_s=float(np.rad2deg(data.qvel[rear["qvel"]][1])),
                       termination=reason)
            for i, side in enumerate(SIDES):
                row.update({f"{side}_hip_deg": float(np.rad2deg(data.qpos[hip_qpos[i]])),
                            f"{side}_hip_velocity_deg_s": float(np.rad2deg(data.qvel[hip_qvel[i]])),
                            f"{side}_assist_deg": float(np.rad2deg(data.qpos[assist["qpos"]][i])),
                            f"{side}_assist_velocity_deg_s": float(np.rad2deg(data.qvel[assist["qvel"]][i])),
                            f"{side}_hip_torque_nm": float(data.qfrc_actuator[hip_qvel[i]]),
                            f"{side}_assist_command_nm": float(torques[i]),
                            f"{side}_assist_torque_nm": float(actual_torque[i]),
                            f"{side}_foot_fz_n": float(fz[i]), f"{side}_contact": int(fz[i]>10.)})
            ...
            if not args.no_realtime:
                time.sleep(max(0., steps*sim_dt - (time.perf_counter()-wall_start)))
