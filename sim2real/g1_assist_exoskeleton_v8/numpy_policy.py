#!/usr/bin/env python3
"""Pure NumPy inference and deployment-state handling for the v8 assist policy."""
...
class V8AssistHistory:
    ...
    def __init__(self) -> None:
        ...
        self.pulse = np.zeros((HISTORY_LENGTH, 20), dtype=np.float32)
        self.next_index = 0
    ...
    def append(
        ...
        if state.shape != (20,) or not np.isfinite(state).all():
            raise ValueError("Expected finite 20-element pulse state")
...
class NumpyAssistPolicy:
    """Float32 v8 MLP with no PyTorch dependency."""
    ...
    def _validate(self) -> None:
        expected_weights = ((256, 650), (64, 256), (16, 64), (2, 16))
        expected_biases = ((256,), (64,), (16,), (2,))
        if self.mean.shape != (INPUT_SIZE,) or self.std.shape != (INPUT_SIZE,):
            raise ValueError("Expected 650-element observation normalization arrays")
    ...
    def forward(self, observation: np.ndarray) -> np.ndarray:
        ...
        if x.ndim not in (1, 2) or x.shape[-1] != INPUT_SIZE:
            raise ValueError(f"Expected observation shape (650,) or (N,650), got {x.shape}")
