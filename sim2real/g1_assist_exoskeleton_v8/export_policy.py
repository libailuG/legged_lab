"""Export verified v8 actor and its training control configuration for NumPy deployment."""
import argparse
import hashlib
import json
...
DEFAULT_RUN = ROOT / 'logs/rsl_rl/g1_assist_exoskeleton_v2_v8_ppo/2026-09-10_13-38-04_motion_guard_cadence'
...
def main():
    ...
    if control['dt'] != .01 or control['torque_limit'] != 10 or int(cfg['observations']['policy']['history_length']) != 25:
        raise ValueError('Expected v8 100 Hz / 25 frames / +/-10 Nm')
    ...
    manifest = dict(control=control, checkpoint=str(checkpoint), source_policy=str(source),
                    ...
                    torque_unit='N.m', history='term-major: 25 q pairs, 25 dq pairs, 25 processed torque pairs, 25 controller-state 20-vectors; oldest first')
