...
try:
    from .numpy_policy import NumpyAssistPolicy, V8AssistHistory
...
class AssistController:
    ...
    def __init__(self, weights=None):
        ...
        if not all(math.isfinite(v) for v in c.values()) or c['dt'] != .01 or c['torque_limit'] != 10:
            raise ValueError('Expected finite v8 parameters / 100 Hz / +/-10 Nm')
    ...
    def step(self, position, velocity, *, enabled=True, timestamp=None):
        ...
        try:
            ...
            self.scalar = float(self.pulse.step(self.raw_action, self.filtered_velocity, p))
