...
ROOT = HERE.parents[1]
spec = importlib.util.spec_from_file_location('sim', ROOT / 'scripts/mujoco/sim2sim_g1_assist_exoskeleton_v2_v8.py')
sim = importlib.util.module_from_spec(spec)
spec.loader.exec_module(sim)

Pair = ct.c_float * 2
class History(ct.Structure):
    _fields_ = [('p', Pair*25), ('v', Pair*25), ('t', Pair*25), ('pulse', (ct.c_float*20)*25), ('index',ct.c_uint)]
class Workspace(ct.Structure):
    _fields_ = [('h',ct.c_float*256), ('s',ct.c_float*650)]
class CController(ct.Structure):
    _fields_ = [('history',History), ('workspace',Workspace), ('obs',ct.c_float*650),
                ('previous',Pair), ('filtered',Pair), ('pulse',ct.c_float*18), ('displacement',Pair), ('ready',ct.c_uint), ('first',ct.c_uint)]
...
class DeploymentTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(['gcc','-std=c99','-O2','-Wall','-Wextra','-Werror','-shared','-fPIC',
                        str(HERE/'c/g1_assist_v8_policy.c'),'-lm','-o',str(lib)], check=True)
        cls.c = ct.CDLL(str(lib))
        cls.c.g1_assist_v8_reset.argtypes = [ct.POINTER(CController), Pair, Pair]
        cls.c.g1_assist_v8_step.argtypes = [ct.POINTER(CController), Pair, Pair, ct.c_int, Pair]
        cls.c.g1_assist_v8_policy_forward.argtypes = [ct.POINTER(ct.c_float),ct.POINTER(ct.c_float),ct.POINTER(Workspace)]
    ...
    def test_network_numpy_c_torch(self):
        ...
        peak = 0.
        for _ in range(200):
            obs = rng.normal(0, 2, 650).astype(np.float32)
            with torch.inference_mode():
                expected = self.jit(torch.from_numpy(obs).unsqueeze(0)).numpy()[0]
            actual = policy(obs)
            self.c.g1_assist_v8_policy_forward(obs.ctypes.data_as(ct.POINTER(ct.c_float)),out,ct.byref(workspace))
    ...
    def test_control_sequence_and_history(self):
        ...
        self.assertEqual(self.c.g1_assist_v8_reset(ct.byref(c),Pair(*p),Pair(*v)),0)
        last = np.zeros(2)
        ...
        for n in range(1500):
            ...
            expected=reference.step(raw,v,p)
            actual=deployment.step(p,v,timestamp=t)
            self.assertEqual(self.c.g1_assist_v8_step(ct.byref(c),Pair(*p),Pair(*v),1,out),0)
    ...
    def test_small_motion_and_fast_cadence_across_backends(self):
        for amplitude, frequency in ((np.deg2rad(.5),2.), (np.deg2rad(1.),4.), (.25,1/.7)):
            d=AssistController();ref=sim.PairedAssistController(d.cfg,.01)
            h=sim.AssistHistory();c=CController();out=Pair()
            p=np.array([.12,-.17],dtype=np.float32);v=np.zeros(2,dtype=np.float32)
            d.reset(p,v);h.reset(p,v)
            self.c.g1_assist_v8_reset(ct.byref(c),Pair(*p),Pair(*v))
            previous=np.zeros(2);peak=0.;adapted=False
            for n in range(700):
                t=n*.01;w=2*np.pi*frequency
                p=np.array([.12+amplitude*np.sin(w*t),-.17-amplitude*np.sin(w*t)],dtype=np.float32)
                v=np.array([amplitude*w*np.cos(w*t),-amplitude*w*np.cos(w*t)],dtype=np.float32)
                if n==0:h.reset(p,v)
                else:h.append(p,v,previous,ref.observation())
                with torch.inference_mode():raw=self.jit(torch.from_numpy(h.observation())[None]).numpy()[0]
                expected=ref.step(raw,v,p);actual=d.step(p,v,timestamp=t)
                self.assertEqual(self.c.g1_assist_v8_step(ct.byref(c),Pair(*p),Pair(*v),1,out),0)
                np.testing.assert_allclose(actual,expected,atol=2e-4,rtol=1e-4)
                np.testing.assert_allclose(list(out),actual,atol=2e-4,rtol=1e-4)
                np.testing.assert_allclose(d.history.observation(),h.observation(),atol=2e-4)
                self.assertLessEqual(np.max(np.abs(actual-previous)),.8001)
                peak=max(peak,np.max(np.abs(actual)))
                adapted |= d.pulse.state[17]>0 and d.pulse.state[8]<.6
                previous=expected
            if amplitude<.03:self.assertEqual(peak,0.)
            else:self.assertTrue(adapted)
    ...
    def test_stop_fault_and_stale_sample(self):
        ...
        self.c.g1_assist_v8_reset(ct.byref(c),Pair(0,0),Pair(1,-1))
        self.assertEqual(self.c.g1_assist_v8_step(ct.byref(c),Pair(float('nan'),0),Pair(1,-1),1,out),-1)
        self.assertEqual(list(out),[0,0]);self.assertEqual(c.ready,0)
        self.assertEqual(self.c.g1_assist_v8_step(ct.byref(c),Pair(0,0),Pair(1,-1),1,out),1)
