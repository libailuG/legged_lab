#!/usr/bin/env python3
"""Generate standalone C99 source/header for the fixed v4 assist policy."""
...
def main() -> None:
    ...
    expected = dict(dt=.01, torque_limit=10., torque_rate_limit=80., motion_filter_time_constant=.05,
                    motion_speed_deadzone=.15, motion_speed_full=.8, pulse_duration=.4, pulse_release_duration=.2, pulse_confirm_time=.03)
    if manifest["control"] != expected:
        raise ValueError("C template requires the verified v4 control parameters")
    ...
    header = """/* Generated fixed-weight v4 assist policy; C99, no heap or file I/O. */
...
"""
    ...
    header_path = output_dir / "g1_assist_v4_policy.h"
    source_path = output_dir / "g1_assist_v4_policy.c"
    ...
    print("Workspace:         2624 bytes per inference context")
