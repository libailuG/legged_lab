#!/usr/bin/env python3
"""Pure NumPy inference and deployment-state handling for the v4 assist policy."""
...
class V4AssistHistory:
    ...
    def __init__(self) -> None:
        ...
        self.pulse = np.zeros((HISTORY_LENGTH, 10), dtype=np.float32)
        self.next_index = 0
    ...
    def append(
        ...
        if state.shape != (10,) or not np.isfinite(state).all():
            raise ValueError("Expected finite 10-element pulse state")
...
class NumpyAssistPolicy:
    """Float32 v4 MLP with no PyTorch dependency."""
    ...
    def _validate(self) -> None:
        expected_weights = ((256, 400), (64, 256), (16, 64), (1, 16))
        expected_biases = ((256,), (64,), (16,), (1,))
        if self.mean.shape != (INPUT_SIZE,) or self.std.shape != (INPUT_SIZE,):
            raise ValueError("Expected 400-element observation normalization arrays")
    ...
    def forward(self, observation: np.ndarray) -> np.ndarray:
        ...
        if x.ndim not in (1, 2) or x.shape[-1] != INPUT_SIZE:
            raise ValueError(f"Expected observation shape (400,) or (N,400), got {x.shape}")
