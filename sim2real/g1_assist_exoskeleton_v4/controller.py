...
try:
    from .numpy_policy import NumpyAssistPolicy, V4AssistHistory
...
class AssistController:
    ...
    def __init__(self, weights=None):
        ...
        if not all(math.isfinite(v) for v in c.values()) or c['dt'] != .01 or c['torque_limit'] != 10:
            raise ValueError('Expected finite v4 parameters / 100 Hz / +/-10 Nm')
        ...
        self.pulse = SinglePulse(c["dt"], c["pulse_duration"], c["pulse_release_duration"],
                                 c["torque_limit"], c["torque_rate_limit"], c["pulse_confirm_time"])
        self.history = V4AssistHistory()
        self.stop()
    ...
    def step(self, position, velocity, *, enabled=True, timestamp=None):
        ...
        try:
            ...
            self.scalar = float(self.pulse.step(self.raw_action, self.filtered_velocity, self.gate))
