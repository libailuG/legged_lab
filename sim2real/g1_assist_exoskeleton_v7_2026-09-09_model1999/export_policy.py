"""Export verified v7 actor and its training control configuration for NumPy deployment."""
import argparse
import hashlib
import json
...
DEFAULT_RUN = ROOT / 'logs/rsl_rl/g1_assist_exoskeleton_v2_v7_ppo/2026-09-09_18-02-01_faster_response'
...
def main():
    ...
    parser.add_argument('--checkpoint', default='model_1999.pt')
    ...
    control = {key: float(action[key]) for key in (
        ...
        'motion_speed_deadzone', 'motion_speed_full', 'pulse_rise_time', 'pulse_release_duration', 'pulse_duration_min', 'pulse_duration_max', 'pulse_confirm_time')}
    ...
    if control['dt'] != .01 or control['torque_limit'] != 10 or int(cfg['observations']['policy']['history_length']) != 25:
        raise ValueError('Expected v7 100 Hz / 25 frames / +/-10 Nm')
    ...
    manifest = dict(control=control, checkpoint=str(checkpoint), source_policy=str(source),
                    ...
                    torque_unit='N.m', history='term-major: 25 q pairs, 25 dq pairs, 25 processed torque pairs, 25 controller-state 12-vectors; oldest first')
