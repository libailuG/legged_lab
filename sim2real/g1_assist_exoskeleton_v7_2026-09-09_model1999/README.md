...
当前权重来自 `2026-09-09_18-02-01_faster_response/model_1999.pt`（2000轮）。来源、SHA256、控制参数见 `weights/manifest.json`。这是本次v7响应优化训练的模型。6～10 N·m是训练奖励目标，部署不强制最低6 N·m，也不额外放大输出。

当前包使用保存的训练配置：速度滤波50 ms、启动确认30 ms、上升200 ms、释放250 ms；对侧抬腿释放仍需60 ms确认，候选状态按60 ms归一化。源代码后来修正的新训练滤波30 ms不适用于这次已训练权重。C生成器会拒绝与当前固定模板不一致的参数，后续若训练参数变化必须同步模板并重新验证。
...
export_policy.py默认使用上面的最新2000轮模型，可通过 `--run RUN_DIR --checkpoint model_N.pt` 指定其他模型。导出后需要重新生成C源码；不要只替换NPZ而继续使用旧C权重。
