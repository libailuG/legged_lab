...
/home/libai/anaconda3/envs/env_isaaclab_0/bin/python \
  sim2real/export_assist_policy_numpy.py

/home/libai/anaconda3/envs/env_isaaclab_0/bin/python \
  sim2real/test_numpy_assist_policy.py
```
...
```bash
/home/libai/anaconda3/envs/env_isaaclab_0/bin/python \
  sim2real/sim2sim_numpy_assist.py --headless --duration 10 --no-realtime
```

去掉 `--headless` 可打开 Viewer。这个 sim2sim 中 PyTorch 只负责原有的 29 关节
步态策略；100 维外骨骼策略完全由 `NumpyAssistPolicy` 计算。

仿真会在 `sim2real/output` 保存完整 CSV 和 PNG。PNG 包含左右髋关节/外骨骼的
...
```bash
gcc -std=c99 -O2 main.c sim2real/c/g1_assist_policy.c -lm
```
...
```bash
python sim2real/export_assist_policy_c.py
```
