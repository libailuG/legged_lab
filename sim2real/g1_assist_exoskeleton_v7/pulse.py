"""NumPy float32 v7 rise-hold-release state machine; local exoskeleton velocity only."""
import math
import numpy as np
...
class SinglePulse:
    def __init__(self, dt=.01, rise_time=.20, release_duration=.25, duration_min=.6,
        ...
        self.scale=np.array([duration_max,1,torque_limit,release_duration,torque_limit,1,1,self.candidate_window,duration_max,.3],dtype=np.float32)
        self.reset()
