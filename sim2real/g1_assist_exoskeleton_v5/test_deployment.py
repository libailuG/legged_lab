...
ROOT = HERE.parents[1]
spec = importlib.util.spec_from_file_location('sim', ROOT / 'scripts/mujoco/sim2sim_g1_assist_exoskeleton_v2_v5.py')
...
class DeploymentTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(['gcc','-std=c99','-O2','-Wall','-Wextra','-Werror','-shared','-fPIC',
                        str(HERE/'c/g1_assist_v5_policy.c'),'-lm','-o',str(lib)], check=True)
        cls.c = ct.CDLL(str(lib))
        cls.c.g1_assist_v5_reset.argtypes = [ct.POINTER(CController), Pair, Pair]
        cls.c.g1_assist_v5_step.argtypes = [ct.POINTER(CController), Pair, Pair, ct.c_int, Pair]
        cls.c.g1_assist_v5_policy_forward.argtypes = [ct.POINTER(ct.c_float),ct.POINTER(ct.c_float),ct.POINTER(Workspace)]
    ...
    def test_network_numpy_c_torch(self):
        ...
        for _ in range(200):
            ...
            actual = policy(obs)
            self.c.g1_assist_v5_policy_forward(obs.ctypes.data_as(ct.POINTER(ct.c_float)),out,ct.byref(workspace))
    ...
    def test_control_sequence_and_history(self):
        ...
        self.assertEqual(self.c.g1_assist_v5_reset(ct.byref(c),Pair(*p),Pair(*v)),0)
        last = np.zeros(2)
        ...
        for n in range(1500):
            ...
            self.assertEqual(self.c.g1_assist_v5_step(ct.byref(c),Pair(*p),Pair(*v),1,out),0)
    ...
    def test_stop_fault_and_stale_sample(self):
        ...
        self.c.g1_assist_v5_reset(ct.byref(c),Pair(0,0),Pair(1,-1))
        self.assertEqual(self.c.g1_assist_v5_step(ct.byref(c),Pair(float('nan'),0),Pair(1,-1),1,out),-1)
        self.assertEqual(list(out),[0,0]);self.assertEqual(c.ready,0)
        self.assertEqual(self.c.g1_assist_v5_step(ct.byref(c),Pair(0,0),Pair(1,-1),1,out),1)
