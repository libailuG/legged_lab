...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="g1_assist_v2_c_test_") as temp_dir:
        ...
        gate_values = tuple(
            library.g1_assist_v2_command_speed_gate(ctypes.c_float(value))
            for value in (0.2, 0.45, 0.7, -0.7)
        )
        if not np.allclose(gate_values, (0.0, 0.5, 1.0, 1.0), rtol=0.0, atol=1e-6):
            raise AssertionError(f"Unexpected C command-speed gate: {gate_values}")
        if library.g1_assist_v2_command_speed_gate(ctypes.c_float(float("nan"))) != 0.0:
            raise AssertionError("Invalid C command must fail safe to zero assist")
    ...
    print("PASS: C99 80 Nm/s slew limiter and final output scale match")
    print("PASS: C99 command-speed gate matches training")
