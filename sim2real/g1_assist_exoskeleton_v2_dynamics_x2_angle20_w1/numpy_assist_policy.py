...
def command_speed_gate(forward_command_m_s: float) -> np.float32:
    """Training-matched smooth gate: zero through 0.2 m/s and full at 0.7 m/s."""
    speed = abs(float(forward_command_m_s))
    phase = np.clip(
        (speed - COMMAND_SPEED_DEADZONE_M_S)
...
class NumpyAssistPolicy:
    ...
    def target_torque(
        self, observation: np.ndarray, forward_command_m_s: float
    ) -> np.ndarray:
        action = np.clip(self.forward(observation), -1.0, 1.0)
        return (
            np.float32(ASSIST_TORQUE_LIMIT_NM)
            * command_speed_gate(forward_command_m_s)
            * action
        )
