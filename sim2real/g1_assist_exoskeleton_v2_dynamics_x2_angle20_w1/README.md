# G1 assist-exoskeleton v2 dynamics-x2-angle20-w1 Sim2Real

本目录对应当前策略：

`logs/rsl_rl/g1_assist_exoskeleton_v2_ppo/2026-09-03_16-14-02_dynamics_x2_angle20_w1/model_1999.pt`
...
- 输出：先用指令速度门控，再将动作限幅到 `[-1, 1]` 后乘以 8 Nm。
- 平滑：每个策略周期最多变化 0.8 Nm，对应 80 Nm/s。
- 速度门控：`|vx|≤0.2 m/s` 时目标助力为零，`|vx|≥0.7 m/s` 时完全开放；中间使用 smoothstep。
- 左右顺序：始终为 `[left, right]`。
...
每段历史均从最旧到最新排列。历史中的力矩是经过指令速度门控和变化率限制、
但尚未乘最终输出系数的平滑策略力矩，
...
python sim2real/g1_assist_exoskeleton_v2_dynamics_x2_angle20_w1/export_assist_policy_numpy.py

conda run --no-capture-output -n env_isaaclab_2 \
python sim2real/g1_assist_exoskeleton_v2_dynamics_x2_angle20_w1/test_numpy_assist_policy.py

conda run --no-capture-output -n env_isaaclab_2 \
python sim2real/g1_assist_exoskeleton_v2_dynamics_x2_angle20_w1/export_assist_policy_c.py

conda run --no-capture-output -n env_isaaclab_2 \
python sim2real/g1_assist_exoskeleton_v2_dynamics_x2_angle20_w1/test_c_assist_policy.py
```
...
gcc -std=c99 -O2 controller.c \
  sim2real/g1_assist_exoskeleton_v2_dynamics_x2_angle20_w1/c/g1_assist_v2_policy.c -lm
```
...
3. 调用 `g1_assist_v2_policy_step()`，传入当前 `vx` 指令，得到经过速度门控和 80 Nm/s 平滑的策略力矩。
