...
DEFAULT_TORCH_POLICY = (
    ...
    "2026-09-03_16-14-02_dynamics_x2_angle20_w1/exported/policy.pt"
)
...
def main() -> None:
    ...
    for speed, expected_gate in expected_gates.items():
        actual_gate = float(command_speed_gate(speed))
        if not np.isclose(actual_gate, expected_gate, rtol=0.0, atol=1e-7):
            raise AssertionError(
                f"Unexpected command gate at vx={speed}: {actual_gate}, expected {expected_gate}"
            )
    print("PASS: Command-speed gate matches the 0.2-to-0.7 m/s smoothstep")
    ...
    for _ in range(20):
        ...
        full_motor = full.step(position, velocity, 0.7)
        half_motor = half.step(position, velocity, 0.7)
    ...
    full.reset(initial_position, initial_velocity)
    for _ in range(20):
        motor = full.step(initial_position, initial_velocity, 0.0)
        if not np.array_equal(motor, np.zeros(2, dtype=np.float32)):
            raise AssertionError(f"Low-speed gate produced non-zero torque: {motor}")
    print("PASS: Low-speed command forces zero assist torque")
