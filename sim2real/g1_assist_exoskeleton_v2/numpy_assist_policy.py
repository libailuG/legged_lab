...
class NumpyAssistPolicy:
    ...
    def target_torque(self, observation: np.ndarray) -> np.ndarray:
        action = np.clip(self.forward(observation), -1.0, 1.0)
        return np.float32(ASSIST_TORQUE_LIMIT_NM) * action
    ...
    def step(self, observation: np.ndarray, previous_torque_nm: np.ndarray) -> np.ndarray:
        return self.slew_limit(previous_torque_nm, self.target_torque(observation))
...
class V2AssistController:
    ...
    def step(self, position_rad: np.ndarray, velocity_rad_s: np.ndarray) -> np.ndarray:
        ...
        smoothed = self.policy.step(observation, self.previous_smoothed_torque_nm)
