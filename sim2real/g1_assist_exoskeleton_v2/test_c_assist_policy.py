...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="g1_assist_v2_c_test_") as temp_dir:
        ...
        if not np.array_equal(smoothed, np.array((0.4, -0.4), dtype=np.float32)):
        ...
        if not np.array_equal(motor, np.array((0.2, -0.2), dtype=np.float32)):
    ...
    print("PASS: C99 40 Nm/s slew limiter and final output scale match")
