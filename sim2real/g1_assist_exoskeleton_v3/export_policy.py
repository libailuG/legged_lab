"""Export verified v3 actor and its training control configuration for NumPy deployment."""
import argparse
import hashlib
import json
...
DEFAULT_RUN = ROOT / 'logs/rsl_rl/g1_assist_exoskeleton_v2_v3_ppo/2026-09-08_10-06-19_paired_assist_10nm_body_pid'
...
def main():
    ...
    parser.add_argument('--checkpoint', default='model_2000.pt')
    ...
    control = {key: float(action[key]) for key in (
        ...
        'motion_speed_deadzone', 'motion_speed_full')}
    ...
    if control['dt'] != .01 or control['torque_limit'] != 10 or int(cfg['observations']['policy']['history_length']) != 25:
        raise ValueError('Expected v3 100 Hz / 25 frames / +/-10 Nm')
    ...
    manifest = dict(control=control, checkpoint=str(checkpoint), source_policy=str(source),
                    ...
                    torque_unit='N.m', history='term-major: 25 q pairs, 25 dq pairs, 25 processed torque pairs; oldest first')
