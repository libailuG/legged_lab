...
ROOT = HERE.parents[1]
spec = importlib.util.spec_from_file_location('sim', ROOT / 'scripts/mujoco/sim2sim_g1_assist_exoskeleton_v2_v3.py')
sim = importlib.util.module_from_spec(spec)
spec.loader.exec_module(sim)

Pair = ct.c_float * 2
class History(ct.Structure):
    _fields_ = [('p', Pair*25), ('v', Pair*25), ('t', Pair*25), ('index',ct.c_uint)]
class Workspace(ct.Structure):
    _fields_ = [('h',ct.c_float*256), ('s',ct.c_float*150)]
class CController(ct.Structure):
    _fields_ = [('history',History), ('workspace',Workspace), ('obs',ct.c_float*150),
                ('previous',Pair), ('filtered',Pair), ('ready',ct.c_uint), ('first',ct.c_uint)]
...
class DeploymentTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(['gcc','-std=c99','-O2','-Wall','-Wextra','-Werror','-shared','-fPIC',
                        str(HERE/'c/g1_assist_v3_policy.c'),'-lm','-o',str(lib)], check=True)
        cls.c = ct.CDLL(str(lib))
        cls.c.g1_assist_v3_reset.argtypes = [ct.POINTER(CController), Pair, Pair]
        cls.c.g1_assist_v3_step.argtypes = [ct.POINTER(CController), Pair, Pair, ct.c_int, Pair]
        cls.c.g1_assist_v3_policy_forward.argtypes = [ct.POINTER(ct.c_float),ct.POINTER(ct.c_float),ct.POINTER(Workspace)]
    ...
    def test_network_numpy_c_torch(self):
        ...
        peak = 0.
        for _ in range(200):
            obs = rng.normal(0, 2, 150).astype(np.float32)
            with torch.inference_mode():
                expected = self.jit(torch.from_numpy(obs).unsqueeze(0)).numpy()[0]
            actual = policy(obs)
            self.c.g1_assist_v3_policy_forward(obs.ctypes.data_as(ct.POINTER(ct.c_float)),out,ct.byref(workspace))
    ...
    def test_control_sequence_and_history(self):
        ...
        self.assertEqual(self.c.g1_assist_v3_reset(ct.byref(c),Pair(*p),Pair(*v)),0)
        last = np.zeros(2)
        ...
        for n in range(1500):
            ...
            else: history.append(p,v,previous_reference)
            with torch.inference_mode():
            ...
            self.assertEqual(self.c.g1_assist_v3_step(ct.byref(c),Pair(*p),Pair(*v),1,out),0)
    ...
    def test_stop_fault_and_stale_sample(self):
        ...
        self.c.g1_assist_v3_reset(ct.byref(c),Pair(0,0),Pair(1,-1))
        self.assertEqual(self.c.g1_assist_v3_step(ct.byref(c),Pair(float('nan'),0),Pair(1,-1),1,out),-1)
        self.assertEqual(list(out),[0,0]);self.assertEqual(c.ready,0)
        self.assertEqual(self.c.g1_assist_v3_step(ct.byref(c),Pair(0,0),Pair(1,-1),1,out),1)
