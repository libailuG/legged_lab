#!/usr/bin/env python3
"""Generate standalone C99 source/header for the fixed v6 assist policy."""
...
def main() -> None:
    ...
    if manifest["control"] != expected:
        raise ValueError("C template requires the verified v6 control parameters")
    ...
    header = """/* Generated fixed-weight v6 assist policy; C99, no heap or file I/O. */
...
void g1_assist_v6_history_reset(
    G1AssistV6History *history,
...
{{
    ...
    for (frame = 0; frame < G1_ASSIST_V6_HISTORY_LENGTH; ++frame) {{
...
void g1_assist_v6_history_append(
    G1AssistV6History *history,
...
{{
    ...
    history->next_index = (index + 1U) % G1_ASSIST_V6_HISTORY_LENGTH;
}}

void g1_assist_v6_history_build_observation(
    const G1AssistV6History *history,
    float observation[G1_ASSIST_V6_INPUT_SIZE])
{{
    unsigned int frame;
    unsigned int side;
    if (history == NULL || observation == NULL) return;
    for (frame = 0; frame < G1_ASSIST_V6_HISTORY_LENGTH; ++frame) {{
        unsigned int source = (history->next_index + frame) % G1_ASSIST_V6_HISTORY_LENGTH;
        for (side = 0; side < 2; ++side) {{
...
void g1_assist_v6_policy_forward(
    const float observation[G1_ASSIST_V6_INPUT_SIZE],
    float action[G1_ASSIST_V6_OUTPUT_SIZE],
    G1AssistV6Workspace *workspace)
{{
    size_t index;
    size_t row;
    ...
    for (index = 0; index < G1_ASSIST_V6_INPUT_SIZE; ++index) {{
    ...
    for (row = 0; row < G1_ASSIST_V6_OUTPUT_SIZE; ++row) {{
...
static float pulse_step(G1AssistV6Controller *c, const float action[2])
{{
    float *s=c->pulse;
...
void g1_assist_v6_stop(G1AssistV6Controller *c, float out[2])
{{
...
int g1_assist_v6_reset(G1AssistV6Controller *c, const float p[2], const float v[2])
{{
    if (c == NULL) return -1;
    g1_assist_v6_stop(c, NULL);
    if (!finite_pair(p) || !finite_pair(v)) return -1;
    g1_assist_v6_history_reset(&c->history, p, v);
    c->first = 1U;
    c->ready = 1U;
    return 0;
}}
int g1_assist_v6_step(G1AssistV6Controller *c, const float p[2], const float v[2], int enabled, float out[2])
{{
    float action[2], state[12], output;
    const float scale[10]={{1.4f,1,10,.25f,10,1,1,.06f,1.4f,.3f}};
    size_t side;
    if (c == NULL || out == NULL) {{
        g1_assist_v6_stop(c, out);
        return -1;
    }}
    out[0] = out[1] = 0;
    if (!enabled || !c->ready) {{
    ...
    if (c->first) g1_assist_v6_history_reset(&c->history, p, v);
    else {{
        for (side=0; side<10; ++side) state[side]=c->pulse[side]/scale[side];
        state[10]=c->filtered_velocity[0]; state[11]=c->filtered_velocity[1];
        g1_assist_v6_history_append(&c->history, p, v, c->previous, state);
    }}
    c->first = 0U;
    g1_assist_v6_history_build_observation(&c->history, c->observation);
    g1_assist_v6_policy_forward(c->observation, action, &c->workspace);
...
"""
    ...
    header_path = output_dir / "g1_assist_v6_policy.h"
    source_path = output_dir / "g1_assist_v6_policy.c"
